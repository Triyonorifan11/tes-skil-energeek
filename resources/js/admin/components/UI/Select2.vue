<template>
    <div>
        <div class="dropdown">
            ...
            <div :class="`select-multiple ${disabled ? 'disabled' : ''}`" v-else>
                <div :class="`input-select form-select ${modelValue.length ? 'p-2' : ''}`" data-bs-toggle="dropdown" data-bs-auto-close="outside" :disabled="disabled" @click="$emit('get-options', searchValue, limit)">
                ...
                <ul class="dropdown-menu select2-multiple-menu" style="width:100%; max-height:50vh; overflow-y:auto; position:relative;">
                    <li class="p-4" v-if="showSearch">
                        <input type="search" v-model="searchValue" @input="search()" class="form-control p-2" placeholder="Search">
                    </li>
                    <template v-if="loading">
