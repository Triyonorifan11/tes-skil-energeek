                    ...
                    <div class="card card-flush mt-5 mb-5 mb-xl-10" id="kt_profile_details_view">
                        <div class="card card-xl-stretch mb-5 mb-xl-8">
                            <div class="card-header border-0 pt-5 align-items-center">
                                <div class="judul">
                                    <h2 class="text-gray-700">Edit Eat & Drink</h2>
                                    <span class="text-gray-700">Lengkapi form dibawah ini untuk mengubah eat & drink</span>
                                </div>
    ...
    export default {
        ...
        methods: {
            ...
            getData(){
                ...
                this.loading = true;
                this.$axios().get(`culinary/${this.$route?.params?.id}`)
                    .then(res => {
                        let data = res?.data?.data;
                        this.loading = false;
                        setTimeout(function(){
                            that.initDropzone(1, data?.culinaryFiles);
                            that.initMap();
                        ...
                        this.form.data.phone = data?.phoneNumber;
                        if(data?.culinaryLanguages.length > 0){
                            this.form.data.culinaries = [];
                        }
                        
                        $.each(data?.culinaryLanguages, function(i,val){
                            that.form.data?.culinaries?.push({
                                language: {id: val?.language?.id, text: val?.language?.name},
                                description: val?.description
                            });
                        });
                        this.form.data.category = [];
                        $.each(data?.culinaryCategories, function(i,val){
                            that.form.data.category.push({id: val?.id, text: val?.name});
                        });
                        this.form.data.openingHours = [];
                        $.each(data?.culinaryOpeningHours, function(i,val){
                            that.form.data.openingHours.push({day: val?.day, startTime: val?.startTime, endTime: val?.endTime});
                        });
                    })
                    .catch(err => {
                        this.$axiosHandleError(err);
            ...
            simpan(){
                ...
                this.$ewpLoadingShow();
                this.$axios().put(`culinary/${this.$route.params.id}`,data)
                .then(res => {
                    let data = res?.data?.data;
