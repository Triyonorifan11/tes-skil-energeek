                    ...
                    <div class="card card-flush mt-5 mb-5 mb-xl-10" id="kt_profile_details_view">
                        <div class="card card-xl-stretch mb-5 mb-xl-8">
                            <div class="card-header border-0 pt-5 align-items-center">
                                <div class="judul">
                                    <h2 class="text-gray-700">Event</h2>
                                    <span class="text-gray-700">Daftar event pada aplikasi Tourism Surabaya - Disbudporapar Kota Surabaya</span>
                                </div>
                                <div class="button d-flex">
                                    <button class="btn btn-outline-info me-3 dropdown-toggle" @click="exportData()">Export</button>
                                    <button class="btn btn-success" @click="$router.push({name: 'a-event-tambah'})">Tambah Data</button>
                                </div>
                            </div>
                            <div class="card-body">
                                            ...
                                            <td valign="middle">
                                                <div class="d-flex align-items-center">
                                                    <div class="d-flex flex-column">
                                                        <h6>{{context?.eventLanguages[0]?.name}}</h6>
                                                        <small class="text-muted">{{ context?.organizer }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td valign="middle" class="text-center">
                                                <h6>{{ $moment(context?.startDate).format('DD/MM/YYYY (HH:mm)') }}</h6>
                                                <span>s/d</span>
                                                <h6>{{ $moment(context?.endDate).format('DD/MM/YYYY (HH:mm)') }}</h6>
                                            </td>
                                            <td valign="middle">
                                            ...
                                            <td valign="middle" class="text-center">
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li><a class="dropdown-item p-2" href="javascript:void(0)" @click="$router.push({path: `event/edit/${context?.id}`})">Edit</a></li>
...
<script>
    ...
    export default {
            ...
            return {
                ...
                tableConfig: {
                    data: {
                        header: [
                            {
                                text: '<input type="checkbox" id="checkfor-all-event" style="width:20px;height:20px;"/>',
                                sortBy: '',
                                sort: false,
                                class: {
                                    column: '',
        ...
        mounted(){
            let that = this;
            $("body").on('click','#checkfor-all-event',function(){                   
        ...
        methods: {
            ...
            getHeader(){
                ...
                this.$axios().get(`menu-header?headerType=event`)
                    .then(res => {
            ...
            checkCheckboxAll(){
                if(this.tableConfig.data.body.length > 0){
                    $("#checkfor-all-event").removeAttr('disabled');
                    if(!this.tableConfig.data.body.filter((e) => !e.isChecklist).length){
                        $("#checkfor-all-event").prop('checked',true); 
                    }else{
                        $("#checkfor-all-event").prop('checked',false); 
                    }
                }
            },
            getDataTable(){
                ...
                $("#checkfor-all-event").attr('disabled',true).prop('checked',false);
                this.$axios().get(`event?language=ID`, {params: this.tableConfig?.config})
                    .then(res => {
                        let data = res?.data?.data;
            ...
            switchStatus(id){
                ...
                this.$ewpLoadingShow();
                this.$axios().put(`event/switch-status/${id}`)
                    .then(res => {
                        $(`.modal`).modal('hide');
            ...
            modalQRCode(data){
                let that = this;
                this.qrcode.data.name = data?.eventLanguages[0]?.name;
                this.qrcode.data.link = `${ this.$baseUrl() }/whats-on/${data?.id}`;
            ...
            changeKominfoAll(){
                ...
                setTimeout(() => {
                    ...
                    let checklist = [];                
                    let isChecked = $("#checkfor-all-event").is(":checked") ? 1 : 0;
                    let mainChecked = $("#checkfor-all-event").is(":checked") ? true : false;
                    ...
                    this.$axios().put(`event/checklist-data`,formData)
                        .then(res => {                        
                            toastr.success('Status berhasil diubah');                        
                            for(let i = 0; i < this.tableConfig.data.body.length; i++){
