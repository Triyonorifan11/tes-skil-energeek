                    ...
                    <div class="card card-flush mt-5 mb-5 mb-xl-10" id="kt_profile_details_view">
                        <div class="card card-xl-stretch mb-5 mb-xl-8">
                            <div class="card-header border-0 pt-5 align-items-center">
                                <div class="judul">
                                    <h2 class="text-gray-700">Edit Event</h2>
                                            ...
                                            <div class="open">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6 row">
                                                        ...
                                                        <div class="col-md-4">
                                                            <app-datepicker v-model:value="form.data.endTimes" type="time" input-class="form-control" input-style="width:100% !important;" format="HH:mm:ss" value-type="HH:mm:ss" placeholder="Jam mulai"></app-datepicker>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
    ...
    export default {
        ...
        methods: {
            getData(){
                ...
                this.$axios().get(`event/${this.$route?.params?.id}`)
                    .then(res => {
                        ...
                        this.loading = false;
                        setTimeout(function(){
                            that.initDropzone(1, data?.eventFiles);
                            that.initMap();
                        ...
                        this.form.data.website = data?.websiteLink;
                        this.form.data.organizer = data?.organizer;
                        this.form.data.category = [];
                        $.each(data?.eventCategories, function(i,val){
                            that.form.data.category.push({id: val?.id, text: val?.name});
                        });
                        this.form.data.events = [];
                        $.each(data?.eventLanguages, function(i,val){
                            that.form.data.events.push({language: {id: val?.language?.id, text: val?.language?.name}, name: val?.name, description: val?.description});
                        });
                        this.form.data.startDates = this.$moment(data?.startDate).format('YYYY-MM-DD');
                        this.form.data.startTimes = this.$moment(data?.startDate).format('HH:mm:ss');
                        this.form.data.endDates = this.$moment(data?.endDate).format('YYYY-MM-DD');
                        this.form.data.endTimes = this.$moment(data?.endDate).format('HH:mm:ss');
                    })
                    .catch(err => {                       
            ...
            simpan(){
                ...
                this.$ewpLoadingShow();
                this.$axios().put(`event/${this.$route.params.id}`,data)
                .then(res => {
                    let data = res?.data?.data;
