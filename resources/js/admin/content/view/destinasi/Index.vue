                    ...
                    <div class="card card-flush mt-5 mb-5 mb-xl-10" id="kt_profile_details_view">
                        <div class="card card-xl-stretch mb-5 mb-xl-8">
                            <div class="card-header border-0 pt-5 align-items-center">
                                <div class="judul">
                                    <h2 class="text-gray-700">Destinasi Wisata</h2>
                                    <span class="text-gray-700">Daftar destinasi wisata pada aplikasi Tourism Surabaya - Disbudporapar Kota Surabaya</span>
                                </div>
                                <div class="button d-flex">
                                    <button class="btn btn-outline-info me-3 dropdown-toggle" @click="exportData()">Export</button>
                                    <button class="btn btn-success" @click="$router.push({name: 'a-destinasi-tambah'})">Tambah Data</button>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <app-data-table :table-config="tableConfig" @get-data="getDataTable">
                                    <template v-slot:rightSearch>
                                        <app-select2 v-model="filter.category" :options="selectList.category.list" :loading="selectList.category.loading" @get-options="getCategory" @change-options="tableConfig.config.categoryId = filter?.category?.id || ''; getDataTable()" placeholder="Semua kategori" :multiple="false"></app-select2>
                                    </template>
                                            ...
                                            <td valign="middle">
                                                <div class="d-flex align-items-center">                                                  
                                                    ...
                                                    <div class="d-flex flex-column">
                                                        <h6>{{ context?.touristDestinationLanguages[0]?.name || '-' }}</h6>
                                                        <small class="text-muted">{{ generatePrice(context) || '-' }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td valign="middle">
                                                <h6>{{ context?.address }}</h6>
                                            </td>
                                            <td valign="middle">
                                                <span class="badge badge-light-info" v-for="(val, i) in context?.tourismCategory">{{ val?.name }}</span>
                                            </td>
                                            <td valign="middle">
                                            ...
                                            <td valign="middle" class="text-center">
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li><a class="dropdown-item p-2" href="javascript:void(0)" @click="$router.push({path: `destinasi/edit/${context?.id}`})">Edit</a></li>
...
<script>
    ...
    export default {
            ...
            return {
                ...
                tableConfig: {
                    data: {
                        header: [
                            {
                                text: '<input type="checkbox" id="checkfor-all-destination" style="width:20px;height:20px;"/>',
                                sortBy: '',
                                sort: false,
                                class: {
                                    column: '',
        ...
        mounted(){
            let that = this;
            $("body").on('click','#checkfor-all-destination',function(){         
                console.log('szzz')       
        ...
        methods: {
            ...
            checkCheckboxAll(){
                if(this.tableConfig.data.body.length > 0){
                    $("#checkfor-all-destination").removeAttr('disabled');
                    if(!this.tableConfig.data.body.filter((e) => !e.isChecklist).length){
                        $("#checkfor-all-destination").prop('checked',true); 
                    }else{
                        $("#checkfor-all-destination").prop('checked',false); 
                    }
                }
            },
            getDataTable(){
                ...
                $("#checkfor-all-destination").attr('disabled',true).prop('checked',false);
                this.$axios().get(`tourist-destination?language=ID`, {params: this.tableConfig?.config})
                    .then(res => {
            ...
            generatePrice(data){
                ...
                if(data?.isFreeForAll){
                    ...
                    $.each(data?.touristDestinationTicketPrices, function(i,val){
                        if(val?.price > 0){
                            harga = that.$rupiahFormat(val?.price);
                            return true;
                        }
                    });
            ...
            switchStatus(id){
                ...
                this.$ewpLoadingShow();
                this.$axios().put(`tourist-destination/switch-status/${id}`)
                    .then(res => {
                        $(`.modal`).modal('hide');
            ...
            modalQRCode(data){
                let that = this;
                this.qrcode.data.name = data?.touristDestinationLanguages[0]?.name;
                this.qrcode.data.link = `${ this.$baseUrl() }/destination/${data?.id}`;
            ...
            renderFile(data){
                ...
                return `${link?.link}?token=${this.token}`;
            },
            ...
            getHeader(){
                ...
                this.$axios().get(`menu-header?headerType=tourist_destination`)
                    .then(res => {
                        let data = res?.data?.data;
            ...
            simpanHeader(){
                ...
                data.headerType = 'tourist_destination';
            ...
            exportData(){
                ...
                this.$ewpLoadingShow();
                this.$axios().get(`tourist-destination/export`, {responseType: 'blob'})
                    .then(res => {
                        let url = window.URL.createObjectURL(new Blob([res?.data]));
                        let el = document.createElement('a');
                        el.href = url;
                        el.download = 'export_destinasi_wisata.xlsx';
                        el.click();
                    })
                    .catch(err => {
                        this.$axiosHandleError(err);
            ...
            changeKominfoAll(){
                ...
                setTimeout(() => {        
                    let checklist = [];                
                    let isChecked = $("#checkfor-all-destination").is(":checked") ? 1 : 0;
                    let mainChecked = $("#checkfor-all-destination").is(":checked") ? true : false;
                    ...
                    this.$axios().put(`tourist-destination/checklist-data`,formData)
                        .then(res => {                        
                            toastr.success('Status berhasil diubah');                        
                            for(let i = 0; i < this.tableConfig.data.body.length; i++){
