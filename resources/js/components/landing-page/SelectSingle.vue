<script setup>
    ...
    const changeSearch = _.debounce(($event) => {
        emit('change-search', $event.target.value, 20)
    }, 800);
            ...
            for (let i = 0; i < conteks.childNodes.length; i++) {
                if (conteks.childNodes[i].classList && conteks.childNodes[i].classList.contains(
                    ...
                    if (conteks.childNodes[i].classList.contains('show')) {
                        ...
                        if (props.serverside) {
                            emit('change-search', search_params.value, 20);
                        }
        ...
        <div id="select-vue-component">
            <div class="dropdown-vue-wrapper">
                <div @click="openDropdown"
                    style="border:1px solid #e4e6ef;border-radius:5px;padding:10px;cursor:pointer"
                    :style="props.disabled ? 'background:#eff2f5 !important;' : 'background:#fff;'"
                    class="d-flex align-items-center justify-content-between">
                    <div style="color:#181c32;font-weight:500;font-size:14px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;display:block"
                        v-if="notEmptyObject(selected_data)">
                        {{selected_data.text}}
                    </div>
                    <div style="color:#a1a5b7;font-weight:500;font-size:14px" v-if="!notEmptyObject(selected_data)">
                    ...
                    <div class="d-flex" style="margin-left:10px;">
                        <div v-if="notEmptyObject(selected_data) && props.show_button_clear && !props.disabled"
                            @click="removeValue()" class="icon-remove-option-select"><i style="font-size:12px;" class="fa fa-times"></i></div>
                        <div><i class="fa fa-chevron-down icon-option-select" style="font-size:12px;"></i></div>
                    </div>
                </div>
                ...
                <div :class="'dropdown-vue-content dissalow-vue-select-option'">
                    <div v-if="show_search" class="dropdown-vue-search-wrap dissalow-vue-select-option"
                        ...
                        <input v-model="search_params" class="form-control dissalow-vue-select-option"
                            @input="changeSearch"
                            style="border:0 !important;outline:none !important;box-shadow:unset !important;font-size:14px !important;"
                            placeholder="Search">
                    </div>
                    <div>
